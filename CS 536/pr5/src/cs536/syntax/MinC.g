...
localVarDecl returns [VarDeclStmt node]
    : ty=type name=IDENT SEMI                  { node = new VarDeclStmt(loc(name), ty,
                                                                        name.getText(), null); }
    | ty=type name=IDENT ass=ASSIGN init=exp SEMI  { node = new VarDeclStmt(loc(ass), ty,
                                                                        name.getText(), init); }
    ;
...
arrayFunctionExp returns [Expression node]
    : e=atomExp                            { node = e; }
        ( l=LBRACKET idx=exp RBRACKET        { node = new ArrayAccessOp(loc(l), node, idx); }
        )*
...
inequalityExp returns [Expression node]
    : e=addExp              { node = e; }
        ( o=LT r=addExp       { node = new LessThanOp(loc(o), node, r); }
        | o=LTE r=addExp      { node = new LessOrEqualOp(loc(o), node, r); }
        | o=GT r=addExp       { node = new GreaterThanOp(loc(o), node, r); }
        | o=GTE r=addExp      { node = new GreaterOrEqualOp(loc(o), node, r); }
        )?
    ;

equalityExp returns [Expression node]
    : e=inequalityExp               { node = e; }
        ( o=EQUAL r=inequalityExp     { node = new EqualsOp(loc(o), node, r); }
        | o=NEQUAL r=inequalityExp    { node = new NotEqualsOp(loc(o), node, r); }
        )*
    ;

andExp returns [Expression node]
    : e=equalityExp             { node = e; }
        ( l=LAND r=equalityExp    { node = new AndOp(loc(l), node, r); }
        )*
    ;

orExp returns [Expression node]
    : e=andExp                  { node = e; }
        ( l=LOR r=andExp          { node = new OrOp(loc(l), node, r); }
        )*
    ;
