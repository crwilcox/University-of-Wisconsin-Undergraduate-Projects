...
localVarDecl returns [VarDeclStmt node]
    : ty=type name=IDENT SEMI                  { node = new VarDeclStmt(loc(name), ty,
                                                                        name.getText(), null); }
    | ty=type name=IDENT a=ASSIGN init=exp SEMI  { node = new VarDeclStmt(loc(a), ty,
                                                                        name.getText(), init); }
    ;
...
globalVarDecl returns [GlobalVarDecl node]
    : ty=type name=IDENT SEMI                  { node = new GlobalVarDecl(loc(name), ty,
                                                                          name.getText(), null); }
    | ty=type name=IDENT a=ASSIGN init=exp SEMI  { node = new GlobalVarDecl(loc(a), ty,
                                                                          name.getText(), init); }
    ;
...
arrayFunctionExp returns [Expression node]
    : e=atomExp                            { node = e; }
        ( b=LBRACKET idx=exp RBRACKET      { node = new ArrayAccessOp(loc(b), node, idx); }
        )*
...
postfixExp returns [Expression node]
    : e=arrayFunctionExp       { node = e; }
        (i=INCR                { node = new IncrementOp(loc(i), node, false); }
        | d=DECR               { node = new DecrementOp(loc(d), node, false); }
        )*
    ;
...
multExp returns [Expression node]
    : e=addrOfExp               { node = e; }
        ( s=STAR r=addrOfExp    { node = new MultiplyOp(loc(s), node, r); }
        | d=DIV r=addrOfExp     { node = new DivideOp(loc(d), node, r); }
        | m=MOD r=addrOfExp     { node = new ModOp(loc(m), node, r); }
        )*
    ;

addExp returns [Expression node]
    : e=multExp                 { node = e; }
        ( p=PLUS r=multExp      { node = new AddOp(loc(p), node, r); }
        | m=MINUS r=multExp     { node = new SubtractOp(loc(m), node, r); }
        )*
    ;

inequalityExp returns [Expression node]
    : e=addExp              { node = e; }
        ( a=LT r=addExp     { node = new LessThanOp(loc(a), node, r); }
        | b=LTE r=addExp    { node = new LessOrEqualOp(loc(b), node, r); }
        | c=GT r=addExp     { node = new GreaterThanOp(loc(c), node, r); }
        | d=GTE r=addExp    { node = new GreaterOrEqualOp(loc(d), node, r); }
        )?
    ;

equalityExp returns [Expression node]
    : e=inequalityExp               { node = e; }
        ( q=EQUAL r=inequalityExp   { node = new EqualsOp(loc(q), node, r); }
        | n=NEQUAL r=inequalityExp  { node = new NotEqualsOp(loc(n), node, r); }
        )*
    ;

andExp returns [Expression node]
    : e=equalityExp             { node = e; }
        ( o=LAND r=equalityExp  { node = new AndOp(loc(o), node, r); }
        )*
    ;

orExp returns [Expression node]
    : e=andExp                  { node = e; }
        ( o=LOR r=andExp        { node = new OrOp(loc(o), node, r); }
        )*
    ;

assignExp returns [Expression node]
    : e=orExp                 { node = e; }
        (a=ASSIGN r=assignExp { node = new AssignOp(loc(a), node, r); }
        )?
    ;
